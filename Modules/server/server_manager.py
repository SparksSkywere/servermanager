...
from Modules.core.common import (
    setup_module_path, get_subprocess_creation_flags, should_hide_server_consoles,
    setup_module_logging, send_command_to_server
)
setup_module_path()

# Import stdin relay starter for persistent command input
try:
...
try:
    from Modules.server.minecraft import (
...
class ServerManager(ServerManagerModule):
    ...
    def start_server_advanced(self, server_name, callback=None):
        ...
        try:
            ...
            with open(stdout_log, 'w', encoding='utf-8') as stdout_file, open(stderr_log, 'w', encoding='utf-8') as stderr_file:
            ...
            # Wrapper scripts (bat/cmd/ps1/sh) can spawn child runtimes that don't
            # stream reliably through PIPE. Writing directly to log files avoids
            # deadlocks from unread pipes and keeps console data available via logs.
            is_wrapper_script = exe_full.lower().endswith(('.bat', '.cmd', '.ps1', '.sh'))

            def _launch_server_process():
                creationflags = get_subprocess_creation_flags(hide_window=hide_consoles, new_process_group=True)
                if is_wrapper_script:
                    stdout_handle = open(stdout_log, 'a', encoding='utf-8', errors='replace')
        ...
        try:
            ...
            if 'StopCommand' in server_config and server_config['StopCommand']:
                try:
                    ...
                    # Shared dispatcher: persistent stdin pipe, queue relay, then stdin relay
                    if not command_sent and send_command_to_server(server_name, stop_cmd):
                ...
                try:
                    if 'StopCommand' in server_config and server_config['StopCommand']:
                        try:
                            ...
                            if send_command_to_server(server_name, stop_cmd):
    ...
    def import_server_config(self, server_name, server_type, install_dir, executable_path, startup_args="", app_id=""):
        ...
        try:
            ...
            if server_type == "Minecraft" and not executable_path:
                ...
                try:
                    from Modules.server.minecraft import MinecraftServerManager
