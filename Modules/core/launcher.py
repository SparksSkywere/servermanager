...
from Modules.core.common import setup_module_path, ServerManagerModule, setup_module_logging, is_admin, get_registry_value, get_registry_values, REGISTRY_PATH
setup_module_path()

logger: logging.Logger = setup_module_logging("Launcher")

class ServerManagerLauncher(ServerManagerModule):
    # Manages child processes detects cluster role (Host/Subhost)
    def __init__(self):
    ...
    def start_tray_icon(self):
        ...
        try:
            tray_script = os.path.join(self.paths["scripts"], "services", "trayicon.py")
            ...
            if tray_process.poll() is not None:
                ...
                logger.error("Tray icon process exited before startup completed; check Services.log for tray icon details")
    ...
    def start_web_server(self):
        ...
        try:
            web_script = os.path.join(self.paths["scripts"], "services", "webserver.py")
            ...
            if web_process.poll() is not None:
                ...
                # Since we're not capturing stdout/stderr, we can't show detailed error info but the process started and exited, which indicates a runtime error
                return False
            ...
            if not self.is_port_open('localhost', web_port):
                ...
                # Since we're not capturing output, we can't get detailed error info but we can try to terminate the non-responsive process
                try:
    ...
    def start_server_automation(self):
        ...
        try:
            automation_script = os.path.join(self.paths["modules"], "services", "server_automation.py")
