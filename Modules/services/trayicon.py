...
from Modules.core.common import setup_module_path, ServerManagerModule, setup_module_logging
setup_module_path()
...
class ServerManagerTrayIcon(ServerManagerModule):
    ...
    def get_running_server_count(self):
        ...
        try:
            return int(self.running_server_count)
        except Exception as e:
    ...
    def create_menu(self):
        ...
        return pystray.Menu(
            pystray.MenuItem(
                lambda item: f"Running servers ({self.get_running_server_count()})",
                lambda: None,
                enabled=False
            ),
            pystray.MenuItem(
                lambda item: f"Webserver ({self.webserver_status})",
                lambda: None,
                enabled=False
            ),
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(webserver_pid_file):
                    with open(webserver_pid_file, 'r', encoding='utf-8') as f:
            ...
            try:
                ...
                if os.path.exists(dashboard_pid_file):
                    with open(dashboard_pid_file, 'r', encoding='utf-8') as f:
                        pid_info = json.load(f)
                        pid = pid_info.get("ProcessId")
                        if pid and self.is_process_running(pid):
            ...
            # Start webserver
            try:
                webserver_script = os.path.join(self.paths["scripts"], "services", "webserver.py")
    ...
    def update_server_status(self):
        # Refresh cached status values for tray menu text.
        try:
            with self._status_lock:
                ...
                try:
                    running_servers = self.server_manager.get_running_servers()
                    self.running_server_count = len(running_servers) if running_servers else 0
                except Exception as e:
                ...
                logger.debug(
                    f"Tray status refreshed: running={self.running_server_count}, webserver={self.webserver_status}"
                )
        except Exception as e:
    ...
    def _status_update_loop(self):
        while not self._status_stop_event.is_set():
            ...
            if self._status_stop_event.wait(10.0):
                break
    ...
    def open_web_interface(self):
        # Open the web interface without blocking the tray event loop.
        try:
            thread = threading.Thread(target=self._open_web_interface_async, daemon=True)
            thread.start()
        except Exception as e:
            logger.error(f"Failed to schedule web interface launch: {str(e)}")
            if self.icon and self.notifications_enabled:
                self.icon.notify(f"Failed to open web interface: {str(e)}", "Server Manager Error")

    def _open_web_interface_async(self):
    ...
    def open_admin_dashboard(self):
        ...
        try:
            ...
            if os.path.exists(admin_pid_file):
                try:
                    with open(admin_pid_file, 'r', encoding='utf-8') as f:
                        pid_info = json.load(f)
            ...
            try:
                ...
                with open(admin_pid_file, 'w', encoding='utf-8') as f:
    ...
    def _stop_child_processes(self):
        ...
        # Best-effort kill for debug windows started outside PID tracking.
        try:
            current_pid = os.getpid()
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                ...
                if self.server_manager_dir and self.server_manager_dir.lower() not in cmdline:
                    continue
                if 'debug_manager.py' in cmdline or 'debug\\debug.py' in cmdline or 'debug/debug.py' in cmdline:
                    try:
                        psutil.Process(proc.info['pid']).terminate()
                        logger.info(f"Terminated debug process (PID {proc.info['pid']})")
                    except Exception as e:
                        logger.error(f"Error terminating debug process {proc.info['pid']}: {e}")
        except Exception as e:
            logger.error(f"Error in debug process cleanup: {e}")
    ...
    def exit_app(self):
        ...
        # Invoke the global stopper so tray Exit behaves the same as Stop-ServerManager.pyw.
        try:
            stop_script = os.path.join(self.paths["scripts"], "services", "stop_servermanager.py")
            ...
            logger.error(f"Failed to invoke global stopper from tray exit: {e}")
        ...
        try:
            ...
            def setup(icon):
                try:
                    ...
                    # Start cached status refresh after the icon is visible.
